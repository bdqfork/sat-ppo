...
class Converter(metaclass=ABCMeta):
    ...
    @abstractmethod
    def reshape_as_input(self, array: np.ndarray, recurrent: bool, device=torch.device):
        """
...
class GraphConverter(Converter):
    ...
    def __init__(self, space: Graph) -> None:
        self.space = space
        self.loss = CrossEntropyLoss()
    ...
    def reshape_as_input(self, array: np.ndarray, recurrent: bool, device=torch.device):
        ...
        array = np.array([utils.batch_graphs([[torch.tensor(i) for i in item] for item in el])
                          for el in array], dtype=np.object)
        array = np.array([[i.to(device) for i in el] for el in array])
        return array

    def action(self, actions: Tensor) -> np.ndarray:
        actions = self.distribution(actions).sample(
        ).cpu().detach().numpy()
        return actions
